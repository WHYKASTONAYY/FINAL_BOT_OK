...
async def send_broadcast(context: ContextTypes.DEFAULT_TYPE, text: str, media_file_id: str | None, media_type: str | None, target_type: str, target_value: str | int | None, admin_chat_id: int):
    ...
    try:
        ...
        for i, user_id in enumerate(user_ids):
            # IMPROVED: Better rate limiting and error handling
            try:
                # IMPROVED: Use send_message_with_retry for better reliability
                if media_file_id and media_type:
                    ...
                    try:
                        ...
                        if "wrong file identifier" in error_str or "file_id" in error_str:
                            # IMPROVED: Fallback to text-only if media fails
                            logger.warning(f"Media file ID invalid for user {user_id}, sending text-only: {media_e}")
                            await send_message_with_retry(bot, user_id, text, parse_mode=None, disable_web_page_preview=True)
                            success_count += 1
                        else:
                            raise media_e
                else:
                ...
                if "chat not found" in error_str or "user is deactivated" in error_str or "bot was blocked" in error_str:
                    ...
                    fail_count += 1
                    # IMPROVED: Try text-only as fallback for other BadRequest errors
                    try:
                        await send_message_with_retry(bot, user_id, text, parse_mode=None, disable_web_page_preview=True)
                        success_count += 1
                        logger.info(f"Broadcast text fallback successful for user {user_id} after media failure")
                    except Exception as fallback_e:
                        logger.error(f"Broadcast text fallback also failed for user {user_id}: {fallback_e}")
                        fail_count += 1

            except telegram_error.Unauthorized:
                logger.info(f"Broadcast skipped for {user_id}: Bot blocked.")
                fail_count += 1
                block_count += 1
                ...
                try:
                    if media_file_id and media_type:
                        ...
                        await bot.send_message(chat_id=user_id, text=text, parse_mode=None, disable_web_page_preview=True)
                    ...
                    fail_count += 1
                    if isinstance(retry_e, (telegram_error.Unauthorized, telegram_error.BadRequest)):
                        block_count += 1

            except Exception as e:
                logger.error(f"Broadcast fail (Unexpected) for {user_id}: {e}", exc_info=True)
                fail_count += 1

            # IMPROVED: Better rate limiting - increased delay to avoid hitting limits
            await asyncio.sleep(0.1)  # 10 messages per second (safer than 20)

            # IMPROVED: More frequent status updates for better monitoring
        ...
        # IMPROVED: Better final summary with success rate
